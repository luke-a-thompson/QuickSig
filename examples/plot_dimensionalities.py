...
plt.plot(dims, sig_dims, 
             color=usyd_ochre, 
             linewidth=3,
             label='Signature Dimension')

# Add log signature line if flag is enabled
if PLOT_LOG_SIGNATURE:
    plt.plot(dims, log_sig_dims, 
             color='blue', 
             linewidth=3,
             label='Log-Signature Dimension')
...
plt.title("Signature Dimension Growth with Path Dimension", 
...
for idx in annotate_indices:
    ...
    plt.annotate(f'{int(round(y))}', (x, y), textcoords="offset points",
                xytext=(0,12), ha='center', va='bottom', fontsize=10,
...
plt.savefig("/home/luke/QuickSig/examples/signature_dimensionalities.pdf", 
