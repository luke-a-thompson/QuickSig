...
def form_butcher_series(
    elementary_differentials: jax.Array,
    ...
    if elementary_differentials.shape[0] != coefficients_flat.shape[0]:
        raise ValueError(
            f"Coefficient count {coefficients_flat.shape[0]} does not match number of basis terms {elementary_differentials.shape[0]}."
        )
    series = jnp.tensordot(coefficients_flat, elementary_differentials, axes=1)
